...
- **Channels:**
  ...
  - Color: `neighbourhood_group_cleansed` (categorical, hue — optional)
  - Filter: `price_is_outlier = False` để loại extreme outliers khỏi view (hoặc giữ lại để thấy outlier)
...
6. Kéo `number_of_reviews` vào **Filters** → Range → minimum = 5.
7. Kéo thêm `price_is_outlier` vào **Filters** → chọn **False** để loại extreme outliers khỏi view.
8. Thêm các field vào **Tooltip**.
9. (Optional) Thêm Reference Line: **Analytics pane → Reference Line** → constant X = median price.
...
4. Kéo `room_type` vào **Color** (grouped bar).
5. Kéo `price_is_outlier` vào **Filters** → **False**.
6. Kéo `MEDIAN(price_per_person)` vào **Label**.
7. Format: Currency ($), 0 decimal.
8. Sort borough theo median (tổng) descending.
...
5. Kéo `year` vào **Color** để vẽ nhiều đường theo năm.
...
8. Sort `Minimum Nights Group` theo thứ tự logic: Short → Medium → Long.
